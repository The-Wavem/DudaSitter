...
export default function AdminAgenda() {
  ...
  const getGoogleCalendarUrl = (appointment) => {
    ...
    const text = encodeURIComponent(
      `${appointment.service.includes("Sitter") ? "Pet Sitter" : "Dog Walker"} - ${appointment.petName} (${appointment.tutorName})`,
    );
  ...
  const summaryToday = appointments.filter(
      ...
      <header className={styles.header}>
        <div>
          ...
          <p className={styles.subtitle}>
            Gerencie seus horários de passeios e visitas.
          </p>
        </div>
      </header>
      ...
      >
        ...
        <div>
          ...
          <p className={styles.summaryText}>
            {summaryToday} compromissos agendados
          </p>
        </div>
      </MotionDiv>
          ...
          <form onSubmit={handleSaveAppointment} className={styles.form}>
            <div className={styles.doubleField}>
              <div className={styles.field}>
                ...
                <input
                  ...
                  required
                  value={newAppTutor}
                  onChange={(event) => setNewAppTutor(event.target.value)}
              ...
              <div className={styles.field}>
                ...
                <input
                  ...
                  required
                  value={newAppPet}
                  onChange={(event) => setNewAppPet(event.target.value)}
                  className={styles.input}
                  placeholder="Pet e Raça"
                />
              </div>
            </div>
            ...
            <div className={styles.field}>
              ...
              <select
                required
                value={newAppService}
                onChange={(event) => setNewAppService(event.target.value)}
                className={styles.select}
              >
                <option value="Pet Sitter (Hospedagem em casa)">
                  Pet Sitter (Hospedagem em casa)
                </option>
                <option value="Dog Walker (Passeios)">
                  Dog Walker (Passeios)
                </option>
              </select>
            </div>
            ...
            <div className={styles.doubleField}>
              <div className={styles.field}>
                ...
                <DatePicker
                  selected={startTime}
                  onChange={(time) => setStartTime(time || new Date())}
                  showTimeSelect
                  showTimeSelectOnly
                  timeIntervals={15}
                  timeCaption="Hora"
              ...
              <div className={styles.field}>
                ...
                <DatePicker
                  selected={endTime}
                  onChange={(time) => setEndTime(time || new Date())}
                  showTimeSelect
                  showTimeSelectOnly
                  timeIntervals={15}
                  timeCaption="Hora"
            ...
            <div className={styles.field}>
              ...
              <textarea
                rows={2}
                value={newAppNotes}
                onChange={(event) => setNewAppNotes(event.target.value)}
              ...
              >
                <div className={styles.timelineTop}>
                  <div>
                    <span className={styles.badge}>
                      {getFriendlyDate(appointment.date, appointment.startTime)}{" "}
                      - {appointment.endTime}
                    </span>
                    ...
                    <p className={styles.timelineService}>
                      {appointment.service}
                    </p>
                  </div>
                ...
                <div className={styles.actions}>
                  {appointment.status === "scheduled" ? (
                    ...
                    <span
                      className={`${styles.statusText} ${appointment.status === "completed" ? styles.statusCompleted : styles.statusCancelled}`}
                    >
